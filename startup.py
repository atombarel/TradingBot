...
def  buy_or_sell(data):
    ...
    # Buy
    if sma[len(sma)-1]<data[len(data)-1] and slowEMA[len(slowEMA)-1]<data[len(data)-1] and slowEMA[len(slowEMA)-1]<fastEMA[len(fastEMA)-1] and rsi[len(rsi)-1] > 50:
        print('Decision => BUY')
        return True
    # Short
    elif sma[len(sma)-1]>data[len(data)-1] and slowEMA[len(slowEMA)-1]>data[len(data)-1] and slowEMA[len(slowEMA)-1]>fastEMA[len(fastEMA)-1] and rsi[len(rsi)-1] < 50:
